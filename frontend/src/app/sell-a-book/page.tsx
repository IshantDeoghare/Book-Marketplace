...
function DropZone({ onFiles, previews, remove, error }: {
  ...
  return (
    <Paper
      ...
      sx={{
        p: 4,
        textAlign: 'center',
        border: `2px dashed ${drag ? theme.palette.primary.main : alpha(theme.palette.text.primary, 0.3)}`,
        borderRadius: 4,
    ...
    >
      ...
      <input
        ...
        accept="image/*"
        multiple
        onChange={e => e.target.files && onFiles(Array.from(e.target.files).slice(0, 5))}
      />
            ...
            <Grid item xs={6} sm={4} md={3} key={src}>
              <Box sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden', '&:hover .overlay': { opacity: 1 } }}>
                ...
                <Box className="overlay" sx={{
                  position: 'absolute', inset: 0, bgcolor: alpha('#000', 0.5),
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  opacity: 0, transition: 'opacity .2s',
                }}>
                  <IconButton color="error" onClick={e => { e.stopPropagation(); remove(i); }}>
                    <Delete />
                  </IconButton>
                </Box>
              </Box>
...
export default function SellABookPage() {
  ...
  const {
    register, handleSubmit, control, watch, setError, clearErrors, formState: { errors }
  ...
  const removePreview = (idx: number) => setPreviews(prev => prev.filter((_, i) => i !== idx));
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select {...field} label="Book Age*">
                      {['0-1 years', '1-3 years', '3-5 years', '5+ years'].map((a) => (
                ...
                render={({ field }) => (
                  <DropZone
                    ...
                    remove={idx => {
                      ...
                      if (remaining.length === 0) field.onChange(null);
                    }}
